	...
	<div class="navbar nav-toolbar">
		<div class="navbar-form">
			<div class="navbar-left">
				<a class="btn btn-primary" href="<?php echo url('admin/user_group/add') ?>">
					<?php echo trans('label.btn_user_group_add') ?>
				</a>
			</div>
	...
	<?php if(isset($result) && $result->total): ?>
		...
		<table class="table table-list table-striped table-hover">
			<thead>
				<tr>
					<th class="td-no">#</th>
					<th><?php echo trans('label.td_group_name') ?></th>
					<th><?php echo trans('label.td_group_permission') ?></th>
			...
			<tbody>
				<?php $i=0+($result->from-1); foreach($result->data as $row): ?>
					...
					<tr>
						...
						<td><?php echo SG_Util::val($row, 'name') ?></td>
						<td><?php echo SG_Util::val($row, 'permissions') ?></td>
						<td class="td-action">
